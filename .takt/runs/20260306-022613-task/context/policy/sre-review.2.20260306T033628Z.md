...
SREレビューで追加適用する判断基準を定義する。

## REJECT 条件

- SLO/SLI 影響が未定義（どの SLI/SLO にどう影響するか不明）
- 障害経路の挙動が未定義（タイムアウト、依存先障害、部分障害時の動作不明）
- リトライ戦略が障害増幅を招く（無制限、バックオフなし、同時再試行集中）
- 影響する SLI の最低限観測点が未定義（必要メトリクス/ログ/アラート欠落）

## Warning 条件

- フェイルオーバーやリカバリ手順の検証が不足
- 監視項目の閾値や運用判断基準が未確定
